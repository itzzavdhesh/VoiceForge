...
export async function speak(request, response, next) {
  try {
    ...
    // Hash the API key to verify ownership on stream request without storing the key longer than necessary.
    const apiKeyHash = apiKey ? createHash("sha256").update(apiKey).digest("hex") : null;
    pendingStreams.set(speechId, { text, voiceId, apiKey, apiKeyHash, mergedSettings, timeout });
...
export async function streamSpeech(request, response, next) {
  try {
    ...
    // Require the caller to provide the same API key that initiated the speech request.
    // This prevents unauthenticated callers from consuming another user's API quota.
    if (!getIsMock()) {
      const providedApiKey = request.get("X-ElevenLabs-Api-Key")?.trim();
      if (!providedApiKey) {
        response.status(401).json({ error: "ElevenLabs API key is required to stream audio. Provide it in the X-ElevenLabs-Api-Key header." });
        return;
      }
      const providedKeyHash = createHash("sha256").update(providedApiKey).digest("hex");
      if (providedKeyHash !== streamData.apiKeyHash) {
        response.status(401).json({ error: "The provided API key does not match the one that initiated this speech request." });
        return;
      }
    }
